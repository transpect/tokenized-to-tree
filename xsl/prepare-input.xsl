...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="prepare-for-full-text-search" as="xs:string" select="'false'"/>
  <xsl:variable name="do-prepare-for-full-text-search" as="xs:boolean" select="$prepare-for-full-text-search = 'true'"/>
  ...
  <xsl:template name="ttt:mark-as-placeholder">
    <xsl:copy>
      ...
      <xsl:if test="name() = 'ttt:normalized-space'">
        <xsl:text> </xsl:text>
      </xsl:if>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="*[ttt:is-para-like(.)]//* | *[ttt:is-para-like(.)]" mode="ttt:add-ids" priority="2">
    <xsl:copy>
      <xsl:call-template name="ttt:lang"/>
  ...
  <xsl:template match="/*/@*" mode="ttt:discard"/>
  
  <xsl:template match="*[ttt:is-para-like(.)]" mode="ttt:count-text" priority="2">
    <xsl:text>&#xa;  </xsl:text>
    <xsl:variable name="text-value-att" as="attribute(ttt:text)">
    ...
    <ttt:para>
      <xsl:copy>
        <xsl:sequence select="$text-value-att"/>
      ...
      <xsl:if test="$do-prepare-for-full-text-search">
        <text>
          <xsl:value-of select="$text-value-att"/>
        </text>
      </xsl:if>
    </ttt:para>
  </xsl:template>
  ...
  <xsl:template match="ttt:normalized-space" mode="ttt:discard">
    <xsl:copy>
      <xsl:apply-templates select="@*, node()" mode="#current"/>
    </xsl:copy>
  </xsl:template>
